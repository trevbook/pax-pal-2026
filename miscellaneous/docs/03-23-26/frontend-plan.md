...
- **SST resources**: `GeminiApiKey` is an `sst.Secret` in `infra/secrets.ts`. S3 Vectors access uses an `sst.Linkable` in `infra/vectors.ts` — this references the externally-provisioned vector bucket/index and grants `s3vectors:QueryVectors` IAM permissions (avoids the `@pulumi/aws-native` gRPC bug). Both are linked to the Nextjs component in `infra/frontend.ts`. Accessed at runtime via `Resource.GeminiApiKey.value` / `Resource.VectorIndex.indexArn`.
...
- **Semantic search** (`app/search/actions.ts` → `semanticSearch()`): Server action. Embeds query via Gemini, queries S3 Vectors for top 30, then hydrates game data from the cached `getAllActiveGames()` list (same as text search — no individual DynamoDB lookups). Converts cosine distance to similarity score (1 - distance/2). Graceful degradation: if Gemini or S3 Vectors fails, returns empty results with error message string.
...
- **Files created**: `lib/vectors.ts` (~95 lines), `app/search/actions.ts` (~120 lines), `components/search-page.tsx` (~290 lines).
- **Files modified**: `app/search/page.tsx` (replaced stub), `infra/vectors.ts` (sst.Linkable for S3 Vectors IAM), `infra/secrets.ts` (GeminiApiKey secret), `infra/frontend.ts` (linked secrets + vectorIndex), `sst.config.ts` (vectors import), `justfile` (simplified setup-search recipe).
